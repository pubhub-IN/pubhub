  ...
  const checkAuth = async () => {
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes("Connection timeout") || error.message.includes("Unable to connect")) {
          setAuthError(`${error.message}. The page will automatically retry when the server is available.`);
        ...
        setAuthError("Failed to verify authentication status. Please try again.");
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Router>
      <Routes>
        <Route
          path="/"
          element={isAuthenticated && user ? <Navigate to="/dashboard" /> : <Hero />}
        />
        <Route
          path="/onboarding"
          element={
        ...
        <Route
          element={
            isAuthenticated && user ? <Layout /> : <Navigate to="/" />
          }
        >
        ...
        <Route 
          path="*" 
          element={<Navigate to={isAuthenticated && user ? "/dashboard" : "/"} replace />} 
        />
      </Routes>
    </Router>
  );
