...
import { ChevronLeft, ChevronRight, Clock, BookOpen, Award, Check, AlertTriangle } from 'lucide-react';
...
export default function LessonPage({ user }: LessonPageProps) {
  ...
  const handleMarkComplete = () => {
    if (user && courseId && lessonId) {
      ...
      if (nextLesson) {
        navigate(`/courses/${courseId}/${nextLesson.moduleId}/${nextLesson.lessonId}`);
      }
    }
  };
  ...
  const goToPrevious = () => {
    if (previousLesson) {
      navigate(`/courses/${courseId}/${previousLesson.moduleId}/${previousLesson.lessonId}`);
    }
  };
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-3xl mx-auto p-6">
              ...
              <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                ...
                {!canComplete && !nextLesson && (
                  <p className="mt-2 text-sm text-amber-600 dark:text-amber-400 flex items-center gap-1">
                    ...
                    <span>Complete at least 80% of lessons to finish the course</span>
                  </p>
                )}
              </div>
            )}
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-6">
              <Clock size={16} />
              <span>Est. time to complete: {currentLesson.timeToComplete} min</span>
            </div>
              ...
              <div className="flex gap-3">
                ...
                {nextLesson ? (
                  ...
                  <>
                    {canComplete ? (
                      <button
                        onClick={() => navigate(`/courses/${courseId}/complete`)}
                        className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md"
                      >
                        <Award size={16} />
